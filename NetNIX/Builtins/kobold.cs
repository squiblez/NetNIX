...
public static class KoboldCommand
{
    ...
    private static int CmdConfig(NixApi api)
    {
        ...
        Console.WriteLine("  use_api_key    Enable API key auth (true/false)");
        Console.WriteLine("  api_key        API key for authenticated endpoints");
    ...
    private static void PrintSettings(KoboldApi kobold)
    {
        ...
        Console.WriteLine("  use_api_key    = " + kobold.UseApiKey);
        Console.WriteLine("  api_key        = " + (kobold.UseApiKey && !string.IsNullOrEmpty(kobold.ApiKey) ? "****" + kobold.ApiKey.Substring(Math.Max(0, kobold.ApiKey.Length - 4)) : "(not set)"));
    }
    ...
    private static void PrintSettingsDetailed(NixApi api, KoboldApi kobold)
    {
        ...
        PrintSettingRow(api, "use_api_key",   kobold.UseApiKey.ToString().ToLowerInvariant());
        PrintSettingRow(api, "api_key",       kobold.UseApiKey && !string.IsNullOrEmpty(kobold.ApiKey) ? "****" + kobold.ApiKey.Substring(Math.Max(0, kobold.ApiKey.Length - 4)) : "(not set)");
    }
    ...
    private static void HandleInlineSet(KoboldApi kobold, string args)
    {
        ...
        switch (key)
        {
            ...
            case "use_api_key":
                kobold.UseApiKey = value.Equals("true", StringComparison.OrdinalIgnoreCase);
                break;
    ...
    private static void PrintUsage()
    {
        ...
        Console.WriteLine("  use_api_key    Enable API key auth   (default: false)");
        Console.WriteLine("  api_key        API key value         (default: empty)");
        Console.WriteLine();
